...
namespace BlackGoldAncientSword.Downloader.Infrastructure
{
    /// <summary>
    /// 下载器进程专用本地日志。<b>本进程与主程序刻意隔离、不引用 Framework，故不能用主程序的 AppLog</b>，
    /// 这里内联一份最小 Serilog 封装，写入与主程序<b>同一日志目录</b>（我的文档\BlackGoldAncientSword\logs），
    /// 文件名前缀 <c>download-</c> 以便与主程序 <c>app-</c>、更新器 <c>update-</c> 日志区分。
    /// <para>
    /// 下载器是排查"下载/安装失败"的唯一现场，故不分 Debug/Release 一律落盘。写入经 Async 包裹不阻塞 UI；
    /// 进程通过 <see cref="!:Environment.Exit"/> 硬退出，必须在退出前调用 <see cref="Flush"/> 确保队列落盘。
    /// </para>
    /// </summary>
    public static class ProcLog
    {
            ...
            try
            {
                ...
                var logger = new LoggerConfiguration()
                    ...
                    .WriteTo.Async(a => a.File(
                        path: Path.Combine(dir, "download-.log"),
        ...
        /// 这样用户在设置页改了日志路径后，下载器也会写到同一个新目录，而非写死的默认目录。
        /// </para>
        /// </summary>

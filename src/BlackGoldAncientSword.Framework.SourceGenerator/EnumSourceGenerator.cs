    ...
    internal class EnumSourceGenerator : IIncrementalGenerator
    {
        ...
        private void GenerateEnumFile(SourceProductionContext context, EnumDefinition def)
        {
            ...
            if (hasBattleApiCodes)
            {
                ...
                methodBuilder.AppendLine("// <auto-generated/>");
                methodBuilder.AppendLine("#nullable enable");
                methodBuilder.AppendLine("using System;");
                methodBuilder.AppendLine();
                methodBuilder.AppendLine($"namespace {def.Namespace}");
                methodBuilder.AppendLine("{");
                ...
                var apiLines = def.Values
                    .Where(v => v.BattleApiCode.HasValue)
                    .Select(v => $"                {v.BattleApiCode!.Value} => {def.Name}.{v.Name},")
                    .ToList();

                foreach (var line in apiLines)
                    methodBuilder.AppendLine(line);

                methodBuilder.AppendLine("                _ => throw new ArgumentOutOfRangeException(nameof(code), $\"未知对局编码: {code}\")");
                methodBuilder.AppendLine("            };");
                methodBuilder.AppendLine("        }");
                methodBuilder.AppendLine("    }");
                methodBuilder.AppendLine("}");

                context.AddSource($"{def.Name}.BattleApiCode.g.cs",
                    SourceText.From(methodBuilder.ToString(), Encoding.UTF8));
            }
        ...
        private class EnumDefinition
        {
            ...
            public int? BattleApiCode { get; set; }

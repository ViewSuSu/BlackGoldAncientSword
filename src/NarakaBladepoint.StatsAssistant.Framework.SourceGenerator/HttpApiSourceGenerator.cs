...
namespace NarakaBladepoint.StatsAssistant.Framework.SourceGenerator
{
    [Generator]
    internal class HttpApiSourceGenerator : IIncrementalGenerator
    {
        ...
        {
            // Read RootNamespace from MSBuild, e.g. "NarakaBladepoint.StatsAssistant.Framework"
        ...
        private void GenerateStaticClient(SourceProductionContext context, ApiDefinitionsRoot root, string httpNs, string generatedNs)
        {
            ...
            if (enumTypeNames.Count > 0)
                sb.AppendLine("using NarakaBladepoint.StatsAssistant.Framework.Core.Consts;");
            sb.AppendLine();
            ...
            sb.AppendLine("            return JsonConvert.DeserializeObject<T>(await r.Content.ReadAsStringAsync(ct))!;");
            sb.AppendLine("        }");
            sb.AppendLine();
            ...
            sb.AppendLine("            r.EnsureSuccessStatusCode();");
            sb.AppendLine("            return JsonConvert.DeserializeObject<TRes>(await r.Content.ReadAsStringAsync(ct))!;");
            sb.AppendLine("        }");
            sb.AppendLine();
        ...
        private void GenerateType(StringBuilder sb, TypeDefinition type, HashSet<string> allTypes, string indent)
        {
            ...
            sb.AppendLine($"{indent}public class {typeName}");

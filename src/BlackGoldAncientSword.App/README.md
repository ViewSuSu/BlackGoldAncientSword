...
助手启动后，首页会**自动检测《永劫无间》游戏进程**。当检测到游戏运行时，会进一步监控游戏日志，实时识别当前所处阶段：

- **等待游戏启动**：永劫无间尚未运行
- **英雄选择中**：已进入英雄选择界面，同时显示 RoomId
- **对局中**：正在进行对局，同时显示 BattleId
- **对局结束**：本局游戏结束

![启动页截图](docs/images/01_home.png)

左侧导航栏可切换至各功能页面：**启动页**、**战绩**、**队伍信息**、**设置**。顶部标题栏显示当前版本号与更新指示器。
...
进入游戏英雄选择界面后，助手会通过 **OCR 技术自动截取屏幕并识别队友昵称**，将队友的赛季战绩数据并排展示，方便快速评估队伍实力。
...
基于 NetSparkle 框架，后台自动检测 GitHub Releases 上的新版本，支持"跳过此版本""稍后提醒""下载并安装"。

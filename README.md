...
> 日常开发在 `main` 分支进行；发版时由 [git-commit](.claude/skills/git-commit/SKILL.md) skill 自动 commit 当前分支 → 切到 `release` 合并源分支 → 直推 `release` 触发 `dotnet-desktop.yml`。`release` 分支保护已解除，允许本地直推，不再走 PR。

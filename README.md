...
战绩数据来源于永劫无间官方 API 接口，程序只负责展示。如果遇到数据查询不到或更新延迟，原因基本出在官方服务器本身，与本程序大概率没啥关系~

**Q：为什么队友识别失败或不准确？**

OCR 识别依赖屏幕截图质量，请确保游戏窗口没有被其他窗口遮挡，英雄选择界面的队友昵称清晰可见。也可以点击「重新识别队伍信息」按钮手动重试。

**Q：客户端为什么有时候会闪退？**

可能原因包括：.NET 运行时异常、游戏日志文件被占用、屏幕捕获 API 冲突等。建议重启应用后重试，如问题持续请在反馈群反馈。
